...
def drawGraphs(graphs,name,xTitle,yTitle,logX,massMin_pos,massMax_pos,outDir):
  ...
  #    g.Draw("PL,same") 
  #  legend.Draw("same")
  #  c2.SaveAs(outDir + "/" +name+"_logY.png","png") 
  #  c2.SaveAs(outDir + "/" +name+"_logY.pdf","pdf") 
 ...
 for i,mMin in enumerate(massMin_pos):
  ...
  for nBins in nBins_pos:  
   for nCats in nCats_pos:     
    ...
    #  cat = cat.replace("  ", "")
     elif len(cat.split())>1 and float(cat.split()[0]) == 1.: 
       nSig = float(cat.split()[4])
       nBkg = float(cat.split()[6])
